package com.dwi.basic.security.permission;
...
public class AuthorizingRealm {
	 ...
	 * @return 是否拥有权限
	 */
	public static boolean hasAllPermission(Collection<String> allPermissions, String[] permissions,
			boolean caseSensitive) {
		if (permissions != null && permissions.length > 0) {
			Collection<WildcardPermission> allPerms = allPermissions.parallelStream()
					.map(perm -> new WildcardPermission(perm, caseSensitive)).collect(Collectors.toList());
	 ...
	 * @return 是否拥有权限
	 */
	public static boolean hasNoPermission(Collection<String> allPermissions, String[] permissions,
			boolean caseSensitive) {
		if (permissions != null && permissions.length > 0) {
			Collection<WildcardPermission> allPerms = allPermissions.parallelStream()
					.map(perm -> new WildcardPermission(perm, caseSensitive)).collect(Collectors.toList());
