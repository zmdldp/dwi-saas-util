package com.dwi.basic.annotation.security;
 ...
 * 2. 通过swagger直接通过后台服务(如：saas-authority-server、saas-file-server)调试时，必须在请求头中传递 userid 参数。 否则会提示 无权限
 * 3. 通过swagger直接通过网关服务(如：saas-gateway-server、saas-zuul-server)调试时，必须在请求头中传递 token 参数 （因为token中含有userid，在网关会将userid解析出来封装到请求头）。否则会提示 无权限。
 * 4. 本地调试时，可以通过全局配置禁用整个服务的 URI权限 校验：saas.security.enabled = false
