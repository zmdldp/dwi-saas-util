package com.dwi.basic.database.datasource;
...
import com.dwi.basic.database.injector.SaasSqlInjector;
import com.dwi.basic.database.mybatis.WriteInterceptor;
import com.dwi.basic.database.mybatis.typehandler.FullLikeTypeHandler;
import com.dwi.basic.database.mybatis.typehandler.LeftLikeTypeHandler;
import com.dwi.basic.database.mybatis.typehandler.RightLikeTypeHandler;
...
/**
 * Mybatis 常用重用拦截器，saas.database.multiTenantType=任意模式 都需要实例出来
 * <p>
 * 拦截器执行一定是：
...
public abstract class BaseMybatisConfiguration {
    ...
    public MybatisPlusInterceptor mybatisPlusInterceptor() {
        ...
        if (MultiTenantType.SCHEMA.eq(this.databaseProperties.getMultiTenantType())) {
            ...
            log.info("检测到 saas.database.multiTenantType=SCHEMA，已启用 SCHEMA模式");
        } else if (MultiTenantType.COLUMN.eq(this.databaseProperties.getMultiTenantType())) {
            log.info("检测到 saas.database.multiTenantType=COLUMN，已启用 字段模式");
            // COLUMN 模式 多租户插件
    ...
    public MetaObjectHandler getMyMetaObjectHandler() {
        return new SaasMetaObjectHandler();
    }
    ...
    @Bean
    @ConditionalOnMissingBean
    public SaasSqlInjector getMySqlInjector() {
        return new SaasSqlInjector();
    }
