package com.dwi.basic.database.mybatis;
...
public class WriteInterceptor extends AbstractSqlParserHandler implements Interceptor {
    ...
    public Object intercept(Invocation invocation) {
        ...
        if (!SpringUtils.getApplicationContext().getEnvironment().getProperty("saas.database.isNotWrite", Boolean.class, false)) {
            return invocation.proceed();
        }
        ...
        if ("0000".equals(tenant) && isWrite && isAuthority) {
            throw new BizException(-1, "演示环境禁止修改、删除重要数据！请登录租户【0000】，账号【saas_pt】创建其他租户管理员账号后测试全部功能");
        }
